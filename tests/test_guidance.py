"""Unit tests for the station-keeping laws, including a closed loop over a toy vehicle.

The toy vehicle is deliberately crude — a point mass that takes up the commanded
velocity instantly, which is all these laws need. It cannot validate tuning against a
real airframe, but it does prove the laws converge and stay bounded, which is what
breaks when the geometry or a sign is wrong. The lagged vehicle the intercept laws are
measured against is exercised in `test_intercept.py`.

The laws that chase a moving target are in `test_intercept.py`, mirroring the split
between `guidance/hold.py`, `guidance/orbit.py` and the intercept files beside them.
"""
