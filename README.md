...
цілі. Літає в ArduPilot SITL, команди по MAVLink, цикл керування 10 Гц. Тільки симулятор,
без заліза. Технічне завдання — [SPEC.md](SPEC.md).
...
| `cli.py` | по одній підкоманді на кожен експеримент |
...
затримка реакції цього зв'язку, а не підібраний коефіцієнт. Винесена вперед точка
...
**Пропорційна навігація** — класичне `a = N · Vc · Ω`. Доки пеленг на ціль не
...
підібране, а не виміряне; докладніше — у розділі про перехоплення.

## Вимірювання

Хост: MacBook (arm64), симулятор у Docker, `SITL_SPEEDUP=1`, home — Київ. Вітер задається
параметрами симулятора (`SIM_WIND_SPD`, `SIM_WIND_DIR`, `SIM_WIND_TURB`) із самого запуску.
...
відростає до 35 м і знову спадає. Це закон зближення, а не супроводу.
...
коли апарат зрушив, а не коли набрав швидкість: 63% заданої він набирає аж за 1.4 с. За
перебором на симуляторі 0.22 с дають 1.50 від мінімуму, 0.66 — 1.10, 1.00 — 1.08, 1.40 —
1.06. Крива вирівнюється приблизно з 0.7 с, і 0.66 стоїть на її початку.
