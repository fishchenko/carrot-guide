...
make test          # 170 тестів без симулятора, ~0.7 с
...
`tests/carrot_guide/` повторює структуру `src/carrot_guide/` каталог у каталог, а останній
каталог у шляху задає тип тесту: `guidance/unit/test_hold.py` і
`guidance/integration/test_hold.py` перевіряють той самий `guidance/hold.py` — перший без
симулятора, другий на SITL. Окремий модуль теж має власний каталог: для `cli.py` це
`cli/unit/` і `cli/smoke/`. Тип вибирається маркером, а не шляхом, і кожен тест несе його
як декоратор — `@pytest.mark.unit` і решта.

- **`-m unit`** — 158 тестів, кожен на одному модулі: математика, парсинг телеметрії,
  метрики, лог, графіки, парсери підкоманд, зліт із відмовами холодного автопілота.
- **`-m component`** — 12 тестів на кількох модулях одразу, але без симулятора: цикл
  керування на fake-апараті та `report`, запущений як із командного рядка. Разом із `unit`
  це 0.7 с — `make test` запускає обидва.
- **`-m smoke`** — 2 тести через встановлений `carrot-guide`: решта імпортує з `src/`,
  тому зламаного встановлення не помітила б.
- **`-m integration`** — 6 тестів на SITL, 3 хв: зліт, утримання з вітром і без, обліт
  кола по всіх чотирьох квадрантах, затримка, перехоплення цілі. Зліт один на всі шість —
  окремий зліт для кожного файлу коштував би дорожче за самі польоти.
- Годинник і `sleep` — параметри і в циклі, і в `MavlinkLink`, тому «десять хвилин на
  20 Гц без дрейфу» чи 90-секундний тайм-аут перевіряються як звичайні швидкі тести.
- Усі doubles лежать у `tests/doubles/` — `messages`, `clocks`, `connection`,
  `vehicles` — навіть ті, що мають одного викликача: у файлі з тестами мають бути
  тести. Решта помічників лежить поруч із тестами, які їх використовують
  (`guidance/toy_vehicle.py`, `commands/parsers.py`), а спільне для кількох пакетів —
  у `tests/runs.py` і `tests/simulator.py`.
