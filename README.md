...
`sleep` прокидає потік пізніше, ніж просили: без цього кожен тік запізнювався на ~10 мс.
...
більше 20% періоду. Останній рядок таблиці обов'язковий: ресинхронізація перебазовує
...
- **один читач сокета.** `recv_match` із фільтром за типом з'їдає інші повідомлення,
  тому всі очікування читають усе, віддають повідомлення трекеру й лише тоді
  фільтрують.
- **тайм-аут телеметрії.** Немає позицій довше за `stale_after_s` — цикл надсилає
  нульову швидкість і падає з помилкою, а не летить за застарілими даними.
